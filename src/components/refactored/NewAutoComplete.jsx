...
const NewAutoComplete = React.forwardRef((props, ref) => {
  ...
  const recordRenderType =
    recordRender && React.isValidElement(recordRender)
      ? 'React'
      : recordRender && typeof recordRender === 'function'
      ? 'function'
      : ''

  const labelRenderType =
    labelRender && React.isValidElement(labelRender)
      ? 'React'
      : labelRender && typeof labelRender === 'function'
      ? 'function'
      : ''
  ...
  const getRecords = argument => {
    ...
    return serviceApi
      ...
      .then(response => {
        ...
        if (data === undefined) {
          message.warning(
            'É possível que o nome da lista não esteja sendo passado corretamente, verifique por gentileza o retorno do end point', 10000
          )
        }
        return data || []
      })
