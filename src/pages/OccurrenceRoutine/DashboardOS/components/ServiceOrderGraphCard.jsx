...
function ServiceOrderGraphCard(props) {
  ...
  const handleClick = (ev) => {
    const { data } = ev;
    if (data && openProposalDetail) {
      openProposalDetail(type, data.id, "typeOSGraph");
    }
  };
          ...
          <Col span={12}>
            ...
            >
              ...
              <Guide>
                <Html
                  position={['50%', '50%']}
                  html={`<div align="center"><span style="font-size: 250%">${totalQuantity}</span><br /><span style="color: gray;">${totalQuantity === 1 ? 'Ordem de serviço' : 'Ordens de serviço'
                    }</span></div>`}
                  alignX="middle"
                  alignY="middle"
                />
          ...
          <Col span={12}>
            {chartData.map(d => (
              <Row
                ...
                type="flex"
                key={d.id}
                onClick={() => openProposalDetail(type, d.id, "typeOSGraph")}
