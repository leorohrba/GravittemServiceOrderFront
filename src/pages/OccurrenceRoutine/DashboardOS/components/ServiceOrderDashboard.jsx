...
export default function ServiceOrderDashboard({ userPermissions }) {
  ...
  const [serviceOrderDetailVisible, setServiceOrderDetailVisible] = useState(false)
  ...
  function getParams(selectedPeriod, rangeDate) {
    ...
    if (filterParams.tipoOSId?.length) {
      params.TipoOSId = filterParams.tipoOSId.join('|')
    }
    if (filterParams.colaboradorId?.length) {
      params.ColaboradorId = filterParams.colaboradorId.join('|')
    }
    if (filterParams.classificacaoOsId?.length) {
      params.ClassificacaoOsId = filterParams.classificacaoOsId.join('|')
    }
    if (filterParams.pessoaId?.length) {
      params.PessoaId = filterParams.pessoaId.join('|')
    }
    if (filterParams.serviceOrderId?.length) {
      params.ServiceOrderId = filterParams.serviceOrderId.join('|')
    }
    if (filterParams.servicoId?.length) {
      params.ServicoId = filterParams.servicoId.join('|')
    }
  ...
  async function openServiceOrderDetail(type, id, source) {
    ...
    try {
      ...
      params.ClassificaoOSId = params.ClassificacaoOsId || []
      ...
      if (source === 'serviceOrderRankingService') {
        ...
        params.ServicoId = service ? service.servicoId.toString() : id.toString()
        if (params.ServicoId === null){
    ...
    if (source === 'serviceOrderRankingClass') {
        ...
        params.ClassificacaoOSId = classifOS ? classifOS.classificacaoOsId.toString() : id.toString()
        if(params.ClassificacaoOSId === null){
          delete params.ClassificacaoOSId
        }
    }
    if (source === 'typeOSGraph') {
        const tipoOS = tipoOSData.find(tipo => tipo.descricao === id)
        params.TipoOSId = tipoOS ? tipoOS.tipoOSId.toString() : id.toString()
        if(params.TipoOSId === null){
          delete params.TipoOSId
        }
    }
    if (source === 'collaboratorGraph') {
        const collab = collaboratorData.find(tipo => tipo.descricao === id)
        params.ColaboradorId = collab ? collab.colaboradorId.toString() : id.toString()
        if(params.ColaboradorId === null){
          delete params.ColaboradorId
        }
    }
    if (source === 'atendimentoRanking') {
        const collab = atendimentoRankingData.find(tipo => tipo.id === id)
        params.ColaboradorId = collab ? collab.id.toString() : id.toString()
        if(params.ColaboradorId === null){
          delete params.ColaboradorId
        }
    }
  ...
  async function getClients() {
    try {
      const response = await apiCRM.get(
        ...
        '/api/crm/person?personType=&addressType=&name=&shortName=&responsibleFranchiseeName=&cityName=&stateAbbreviation=&cnpj=&cpf=&queryOperator=like&qualificacaoId=&origemContatoId=&segmentoMercadoId=&areaNegocioId=&userName=&status=&hasTasks=&attribute=&email=&getPersonDetails=true',
      )
      setFilterClients(
  ...
  async function getTypes() {
    try {
      ...
      setFilterTypes(
        response.data.ServiceOrderTypes?.map(c => ({ label: c.Description, value: c.ServiceOrderTypeId })),
      )
    } catch (error) {
      console.error('Error fetching horas apontadas data:', error)
    }
  ...
  async function fetchOS(params) {
    try {
      ...
      setQtOSAguardando(response.data.osAgliquidacao)
