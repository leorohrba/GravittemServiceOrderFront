...
import NewAttendanceChannelModalForm from './NewAttendanceChannelModalForm'

function NewAttendanceChannelModal({
  form,
  ...
  async function getAttendanceChannel() {
    ...
    try {
      ...
      setLoading(false)
      const { data } = response
      if (data.isOk && data.canalAtendimento.length > 0) {
        setEditData(data.canalAtendimento[0])
      } else if (data.isOk && data.canalAtendimento.length === 0) {
        message.error(formatMessage({ id: 'recordNotFound' }))
        setVisibleAttendanceChannelModal(false)
      } else {
  ...
  function handleSave(addAnother) {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (err) {
        ...
        saveAttendanceChannel(addAnother)
      }
    })
  }

  async function saveAttendanceChannel(addAnother) {
    setIsSaving(true)
    setLoading(true)
    const attendanceChannelBody = {
      id,
  ...
  return (
    <Modal
      title={formatMessage({
        id: 'occurrenceRoutine.attendanceChannel.attendanceChannel',
      })}
      visible={visibleAttendanceChannelModal}
      centered
      onCancel={() => setVisibleAttendanceChannelModal(false)}
      footer={
        <Row type="flex">
          ...
          <Button
            ...
            onClick={() => setVisibleAttendanceChannelModal(false)}
          >
...
NewAttendanceChannelModal.propTypes = {
  id: PropTypes.string,
  refreshData: PropTypes.func,
  userPermissions: PropTypes.array,
  form: PropTypes.any,
  setVisibleAttendanceChannelModal: PropTypes.any,
  visibleAttendanceChannelModal: PropTypes.any,
  setId: PropTypes.func,
}

const WrappedNewAttendanceChannelModal = Form.create()(
  NewAttendanceChannelModal,
)
export default WrappedNewAttendanceChannelModal
